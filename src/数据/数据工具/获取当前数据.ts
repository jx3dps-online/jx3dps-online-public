import { 获取心法数据 } from '@/心法模块/模块工具/获取心法数据'
import { 数据类型 } from '@/@types/数据'
import { 获取团队增益数据 } from '../团队增益'
...
let 缓存数据: 数据类型 | null = null

export const 初始化心法数据 = async (心法) => {
  const 心法数据 = await 获取心法数据(心法)
  const 功法 = 心法数据?.功法
  const 部分数据功法 =
    功法 === '防御'
      ? ['铁骨衣', '铁牢律']?.includes(心法数据?.名称)
        ? '防御_外功'
        : '防御_内功'
      : 功法
  ...
  const 结果数据: 数据类型 = {
    ...心法数据,
    团队增益: 获取团队增益数据(部分数据功法, 心法数据?.名称),
    阵眼: 获取当前心法阵眼(心法数据, 部分数据功法),
    小药小吃: 小药小吃数据?.[部分数据功法] || [],
...
const 获取当前数据 = (): 数据类型 => {
  if (缓存数据) {
    if (globalThis?.心法 || globalThis?.xf) {
      if (globalThis?.心法 === 缓存数据?.名称 || globalThis?.xf === 缓存数据?.简写) {
        return 缓存数据
      }
    } else {
      return 缓存数据
