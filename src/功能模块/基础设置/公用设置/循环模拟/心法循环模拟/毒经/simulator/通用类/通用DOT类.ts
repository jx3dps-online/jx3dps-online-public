...
class 通用DOT类 extends 技能统一类 {
  ...
  更新待生效数据(当前层数: number, DOT数据: DotDTO) {
    ...
    const 降厄加速值 = this.模拟循环?.当前自身buff列表?.['降厄']?.当前层数 ? 512 : 0
    const 狂暴加速值 = this.模拟循环?.当前自身buff列表?.['蛊虫狂暴']?.当前层数 ? 102 : 0
    const 额外加速值 = 降厄加速值 + 狂暴加速值

    const 实际初次频率 = DOT是否吃加速
      ? 获取实际帧数(DOT数据.初次频率 || DOT数据.伤害频率, 循环加速值, 0, 额外加速值)
      : DOT数据.初次频率 || DOT数据.伤害频率

    const 实际伤害频率 = DOT是否吃加速
      ? 获取实际帧数(DOT数据.伤害频率, 循环加速值, 0, 额外加速值)
      : DOT数据.伤害频率
