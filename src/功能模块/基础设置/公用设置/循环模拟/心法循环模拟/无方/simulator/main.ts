...
import 商陆缀寒 from './技能类/商陆缀寒'
import 钩吻断肠 from './技能类/钩吻断肠'
import 川乌射罔 from './技能类/川乌射罔'
import 沾衣未妨 from './技能类/沾衣未妨'
import 银光照雪 from './技能类/银光照雪'
import 含锋破月 from './技能类/含锋破月'
import 飞叶满襟 from './技能类/飞叶满襟'
import 苍棘缚地 from './技能类/苍棘缚地'
...
class 循环主类 {
  ...
  消耗内力(数值) {
    ...
    if (this.技能类实例集合.千枝绽蕊?.千枝绽蕊状态校验() && this.角色状态信息.内力 <= 5) {
      this.卸除buff?.({ 名称: '千枝绽蕊', 对象: '自身', 卸除层数: 1 })
      this.删除待生效事件队列(`千枝绽蕊_秒耗`)
      this.技能类实例集合.千枝伏藏.获得千枝伏藏()
    }
  }
  // 校验奇穴是否存在
  校验奇穴是否存在(待判断奇穴) {
      ...
      const 轮训执行事件 = () => {
        if (this.待生效事件队列[0]?.事件时间 <= 结算判断时间) {
          ...
          if (当前事件.事件名称?.includes('技能读条')) {
            ...
            this.技能类实例集合?.['苍棘缚地']?.苍棘缚地攻击()
          } else if (当前事件.事件名称 === '蛰伏结束') {
            this.技能类实例集合?.['苍棘缚地']?.蛰伏结束()
          }
          this.待生效事件队列?.shift()
          if (this.待生效事件队列.length) {
            轮训执行事件()
