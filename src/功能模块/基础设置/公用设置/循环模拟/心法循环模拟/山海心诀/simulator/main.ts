...
import 唤灵印 from './技能类/唤灵印'
import 下马 from './技能类/下马'
import 游雾乘云 from './技能类/游雾乘云'
...
class 循环主类 {
  ...
  技能成功开始释放(当前技能: 循环基础技能数据类型, 技能实例, 是否为读条技能, 释放索引) {
    if (!(释放索引 === 0 && (当前技能.技能名称 === '饮羽簇' || 当前技能.技能名称 === '聚势摧霆'))) {
      this.增加技能GCD(当前技能)
    }
    if (当前技能.技能名称 === '饮羽簇') {
      // 读条引在读条结束以后进入CD
    ...
    if (this.待生效事件队列.length) {
      ...
      const 轮训执行事件 = () => {
        if (this.待生效事件队列[0]?.事件时间 <= 结算判断时间) {
          ...
          } else if (当前事件.事件名称?.includes('卸除换灵印')) {
            if (当前事件?.事件备注?.本次召唤宠物) {
              this?.技能类实例集合?.唤灵印?.卸除换灵印(当前事件?.事件备注?.本次召唤宠物)
            }
          } else if (当前事件.事件名称?.includes('下马')) {
            this?.技能类实例集合?.下马?.命中()
          }
          this.待生效事件队列?.shift()
          if (this.待生效事件队列.length) {
  ...
  棘矢引爆贯穿判定(来源) {
    if (this.校验奇穴是否存在('棘矢')) {
      if (来源 === '没石饮羽' || this.角色状态信息.箭袋信息?.length === 1) {
  ...
  金乌箭判定(箭形态?) {
    ...
    if (箭形态 === 箭形态枚举.红箭 || 下一根箭形态 === 箭形态枚举.红箭) {
      ...
      this.技能造成伤害('破')
    } else if (箭形态 === 箭形态枚举.紫箭 || 下一根箭形态 === 箭形态枚举.紫箭) {
      this.技能类实例集合.贯穿.获得和刷新贯穿('金乌')
      this.技能造成伤害('破')
    }
  }
  ...
  标鹄判定() {
    ...
    const 当前连珠层数 = this.当前目标buff列表?.['连珠']?.当前层数 || 0
    // 引爆标鹄
    if (当前标鹄层数 === 4) {
      // 引爆上一层贯穿
  ...
  召唤宠物(召唤来源, 额外buff?) {
    ...
    this.技能类实例集合?.唤灵印?.获得唤灵印(本次召唤宠物, 召唤来源)
  }
    ...
    if (this.校验奇穴是否存在('襄尺')) {
      ...
      if (buff承契数据) {
        ...
        if (buff承契数据?.当前层数 === 5 && 剩余帧 >= 每秒郭氏帧 * 8) {
          this.技能类实例集合.金乌见坠.减少调息时间(每秒郭氏帧 * 3)
        }
      }
    }
    ...
    const 找出所有读条技能的区间: Array<{ 开始时间: number; 结束时间: number; 是否读条: boolean }> =
      所有释放技能数组
        .map((item, index) => {
          ...
          const 是否读条 = item?.日志 && ['朝仪万汇', '聚势摧霆']?.includes(item?.日志)
          return {
            开始时间: item?.日志时间,
