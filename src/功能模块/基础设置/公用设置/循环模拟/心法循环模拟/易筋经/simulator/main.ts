...
import 普渡四方 from './技能类/普渡四方'
import 韦陀献杵 from './技能类/韦陀献杵'
import 横扫六合 from './技能类/横扫六合'
import 捕风式 from './技能类/捕风式'
import 捉影式 from './技能类/捉影式'
import 守缺式 from './技能类/守缺式'
import 拿云式 from './技能类/拿云式'
import 擒龙决 from './技能类/擒龙决'
import 罗汉金身 from './技能类/罗汉金身'
import 二业依缘 from './技能类/二业依缘'
import 换行 from './技能类/换行'
import 进入斩杀 from './技能类/进入斩杀'
import 摩诃无量 from './技能类/摩诃无量'
import 触发橙武 from './技能类/触发橙武'
import 横扫六合DOT from './DOT类/横扫六合'
...
class 循环主类 {
  ...
  constructor(props: SimulatorCycleProps) {
    ...
    this.是否能够使用拿云式 = !!props.是否能够使用拿云式
  ...
  技能造成伤害(
    ...
    if (this.校验奇穴是否存在('三生')) {
      const 当前禅那 = this.角色状态信息.禅那
      if (当前禅那) {
        总增益列表 = 总增益列表?.concat([`三生·${当前禅那}`])
      }
    }
    if (this.校验奇穴是否存在('金刚怒目') && this.当前自身buff列表['罗汉金身']?.当前层数) {
      总增益列表 = 总增益列表?.concat(['罗汉金身_金刚怒目'])
    }
    if (this.校验奇穴是否存在('系珠') && this.当前自身buff列表['擒龙']?.当前层数) {
      总增益列表 = 总增益列表?.concat(['擒龙_系珠'])
    }
    if (this.角色状态信息.状态 === '伏魔') {
      总增益列表 = 总增益列表?.concat(['伏魔'])
    }
  ...
  检查GCD(当前技能: 循环基础技能数据类型, 技能实例, i) {
    let GCD = 0
    // const 无诤 = this.当前自身buff列表?.无诤
    // if (当前技能.技能类型 === '龙爪功' && this.当前自身buff列表?.无诤?.当前层数 && this.校验奇穴是否存在('无执')) {
    //   const 上一个技能 = this.测试循环[i - 1]
    //   if (上一个技能 === '横扫六合') {
    //     return 3
    //   }
    //   return 每秒郭氏帧
    // } else if (当前技能.技能类型 !== '龙爪功' && 无诤?.当前层数 && this.校验奇穴是否存在('无执')) {
    //   return (无诤?.刷新时间 || 0) + (无诤?.最大持续时间 || 0) - this.当前时间
    // } else
    if (技能实例?.检查GCD) {
  ...
  技能释放前(当前技能: 循环基础技能数据类型, 技能实例, i) {
    ...
    const 延迟等待 = i > 0 && !是否是倒读条技能 && GCD ? this.网络延迟 : 0
  ...
  技能释放后(
    ...
    if (当前技能.技能类型 === '龙爪功') {
      this.连续龙爪功次数++
      if (this.连续龙爪功次数 >= 3) {
        const 贪破层数 = this.当前自身buff列表.贪破?.当前层数 || 0
        this.卸除buff({ 名称: '贪破', 对象: '自身', 卸除层数: 10 })
        this.技能造成伤害('破', 贪破层数, [], undefined, false, 3)
        this.添加buff({ 名称: '贪破', 对象: '自身', 新增层数: 1 })
        this.添加战斗日志({
          日志: '连续释放3次龙爪功 造成贪破伤害',
          日志类型: '技能释放结果',
        })
      ...
      !['罗汉金身', '二业依缘', '擒龙决', '进入斩杀', '触发橙武', '换行', '特效腰坠'].includes(
        当前技能.技能名称,
      )
    ) {
      this.连续龙爪功次数 = 0
  ...
  DOT结算与更新() {
    ...
    this.技能类实例集合?.横扫六合DOT?.结算横扫六合DOT伤害()
  }
  ...
  技能释放校验(技能实例, 当前技能) {
    const 无诤 = this.当前自身buff列表?.无诤
    if (当前技能.技能类型 !== '龙爪功' && 无诤?.当前层数 && this.校验奇穴是否存在('无执')) {
      if (无诤?.当前层数 && this.校验奇穴是否存在('无执')) {
        const 剩余时间 = (无诤?.刷新时间 || 0) + (无诤?.最大持续时间 || 0) - this.当前时间
        if (剩余时间 < 每秒郭氏帧) {
          return {
            可以释放: true,
          }
        ...
        return {
          可以释放: false,
          异常信息: { 信息: ERROR_ACTION.缴械无法释放?.信息 },
        }
      }
    }
    ...
    if (this.待生效事件队列.length) {
      ...
      const 轮训执行事件 = () => {
        if (this.待生效事件队列[0]?.事件时间 <= 结算判断时间) {
          ...
          } else if (当前事件.事件名称?.includes('擒龙回豆')) {
            this.技能类实例集合?.擒龙决?.擒龙回豆()
          ...
          } else if (当前事件.事件名称?.includes('六合棍意结算')) {
            this?.技能类实例集合?.['横扫六合']?.六合棍意结算()
