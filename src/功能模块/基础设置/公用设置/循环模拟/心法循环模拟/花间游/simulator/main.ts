...
import 水月无间 from './技能类/水月无间'
import 阳明指 from './技能类/阳明指'
import 快雪时晴 from './技能类/快雪时晴'
import 商阳指 from './技能类/商阳指'
import 兰摧玉折 from './技能类/兰摧玉折'
import 钟林毓秀 from './技能类/钟林毓秀'
import 乱洒青荷 from './技能类/乱洒青荷'
...
import DOT_钟林毓秀 from './DOT类/钟林毓秀'
...
class 循环主类 {
  ...
  constructor(props: SimulatorCycleProps) {
    ...
    this.角色状态信息 = {
      墨意: 60,
      临源充能: Math.floor((props?.起手临源 || 0) / 5) || 0,
      临源豆: props?.起手临源 ? (props?.起手临源 || 0) % 5 || 0 : 0,
    }
    this.待生效事件队列 = []
    // 模拟初始化
    this.初始化技能实例类()
  ...
  初始化技能实例类() {
    this.技能类实例集合 = {
      ...
      DOT_兰摧玉折: new DOT_兰摧玉折(this),
      DOT_钟林毓秀: new DOT_钟林毓秀(this),
      换行: new 换行(this),
      触发橙武: new 触发橙武(this),
      特效腰坠: new 特效腰坠(this),
    }
  ...
  技能造成伤害(
    ...
    const 有关的buff列表 =
      技能增益列表
        ...
        ?.map((item) => {
          if (来源?.includes('DOT')) {
            if (item?.增益名称 === '倚天' && 来源?.includes('DOT')) {
              return '倚天_常驻'
            } else {
              return item.增益名称
            }
  ...
  DOT结算与更新() {
    this.技能类实例集合?.DOT_商阳指?.结算商阳指伤害()
    this.技能类实例集合?.DOT_钟林毓秀?.结算钟林毓秀伤害()
    this.技能类实例集合?.DOT_兰摧玉折?.结算兰摧玉折伤害()
  }
    ...
    if (this.待生效事件队列.length) {
      ...
      const 轮训执行事件 = () => {
        if (this.待生效事件队列[0]?.事件时间 <= 结算判断时间) {
          ...
          } else if (当前事件.事件名称 === '丹青判定') {
            this.技能类实例集合?.['丹青']?.丹青命中()
          } else if (当前事件.事件名称 === '雨墨判定') {
            this.技能类实例集合?.['丹青']?.雨墨命中()
          }
          this.待生效事件队列?.shift()
          if (this.待生效事件队列.length) {
            轮训执行事件()
