...
const 装备特效加成 = (
  ...
  if (装备增益?.特效_40794) {
    // 等级1/3破防 2/4会心
    let 算法结果
    if ([1, 3]?.includes(装备增益?.特效_40794)) {
      const 等级 = 装备增益?.特效_40794 === 1 ? 1 : 2
      算法结果 = 特效_40794破防算法(实际面板, 实际计算属性, 等级)
    } else {
      const 等级 = 装备增益?.特效_40794 === 2 ? 1 : 2
...
const 特效_40794破防算法 = (
  ...
  const 无双加成值 = [0, 93, 155]
  const 破防加成值 = [0, 12152, 20148]
  if (最终无双百分比 <= 0.9) {
    return {
      计算人物面板,
      最终计算属性: {
...
const 项链破防转攻击算法 = (人物面板: 角色基础属性类型, 加成等级: number): 角色基础属性类型 => {
  ...
  const 等级加成数值 = 是否为内功 ? [33, 35, 38, 42, 46, 50] : [30, 32, 35, 38, 41, 45]
  let 加成层数 = 0
  if (加成等级 === 1) {
    ...
    加成层数 = 获取项链特效层数(破防等级 / 6864)
  } else {
    加成层数 = 获取项链特效层数(破防等级 / 7456)
  }
...
const 项链会心转会效算法 = (人物面板: 角色基础属性类型, 加成等级): 角色基础属性类型 => {
  ...
  if (加成等级 === 1) {
    ...
    加成层数 = 获取项链特效层数(会心等级 / 6006)
  } else {
    加成层数 = 获取项链特效层数(会心等级 / 6526)
  }
