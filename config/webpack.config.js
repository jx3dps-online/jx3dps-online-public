...
module.exports = function (webpackEnv) {
  ...
  const isAnalyzer = process.env.BUILD_ANALYZER === 'true'
  ...
  return {
    ...
    optimization: {
      ...
      splitChunks:
        isEnvProduction && !isGetDpsMoment
          ? {
              chunks: 'all',
              minSize: 30000,
              minChunks: 1,
              ...
              cacheGroups: {
                ...
                xinfaTools: {
                  test: /心法模块\/模块工具\/.*/, // 不排除 index.ts
                  name: 'kongfu_tools',
                  enforce: true,
                  priority: 20,
                },
                xinfaCommon: {
                  test: /心法模块\/统一数据\/.*/, // 不排除 index.ts
                  name: 'kongfu_common',
                  enforce: true,
                  priority: 20,
                },
                ...
                xinfa: {
                  test: new RegExp(
                    `[\\\\/]心法模块[\\\\/]心法[\\\\/](${Object.keys(xinfaNameMap).join(
                      '|',
                    )})[\\\\/]`,
                  ),
                  name(module) {
                    ...
                    return `xinfa-${xinfaNameMap[match[1]]}`
                  },
                  enforce: true,
                  priority: 20,
                ...
                simulator: {
                  test: /[\\/]心法循环模拟[\\/](.*?)[\\/]/,
                  name(module) {
                    const pathMatch = module.identifier().match(/心法循环模拟[\\/](.*?)[\\/]/)
                    return xinfaNameMap[pathMatch[1]]
                      ? `simulator-${xinfaNameMap[pathMatch[1]]}`
                      : 'simulator-unknown'
                  },
                  priority: 20,
                  enforce: true,
                ...
                // 进一步拆分 系统工具 配装器 其他
                功能模块: {
                  test: /[\\/]功能模块[\\/](?!基础设置[\\/]公用设置[\\/]循环模拟[\\/]心法循环模拟)/,
                  name: 'feature',
                  priority: 20,
                  enforce: true,
                },
    ...
    //     {
    //       context: ['/api'],
    //       target: 'https://dps.arkwish.com/api',
    //       changeOrigin: true,
    //     }
    //   ]
    // },
